...
export default function PenerimaBarangPage() {
  ...
  useEffect(() => {
    ...
    if (savedList) {
      try {
        setPenerimaList(JSON.parse(savedList));
      } catch {
        setPenerimaList([]);
      }
    }
  ...
  const handleSelect = (penerima: Penerima) => {
    // Simpan penerima yang dipilih ke localStorage
    localStorage.setItem("selectedPenerima", JSON.stringify(penerima));

    // Arahkan kembali ke halaman pesan
  ...
  return (
    <div className="min-h-screen bg-white text-black">
      ...
      <div className="sticky top-0 z-30 flex items-center gap-3 px-4 py-3 bg-white border-b">
        ...
        <h1 className="text-sm font-semibold">Informasi Penerima Barang</h1>
      </div>
