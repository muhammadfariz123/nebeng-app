...
interface BarangUmum {
  nama: string;
  kategori: string;
  poin: number;
}

export default function KonfirmasiBarangUmumPage() {
  ...
  const harga = Number(params.get("harga")) || 50000;

  const barang: BarangUmum = {
    nama: "Barang Umum",
    kategori: "Barang Kecil",
    poin: 10,
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f5f7fa]">
      ...
      <div className="bg-blue-600 text-white px-4 pt-10 pb-4 flex items-center">
        <Link href="/customer/barang-umum/pesan">
      ...
      <div className="mx-4 mt-6 bg-white rounded-2xl shadow p-4">
        ...
        <div className="flex justify-between text-xs text-gray-500">
          <span>{barang.kategori} (MAX 50 Kg)</span>
          <span>Rp {harga.toLocaleString("id-ID")}</span>
        </div>
      </div>
      ...
      <div className="p-4 mt-auto">
        <button
          ...
          onClick={() =>
            router.push("/customer/barang-umum/konfirmasi-pesanan")
          }
        >
          BAYAR
        </button>
