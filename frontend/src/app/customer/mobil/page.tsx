...
export default function MobilSearchPage() {
  ...
  const handleTukar = () => {
    ...
    router.replace(
      `/customer/mobil?dari=${encodeURIComponent(newDari)}&ke=${encodeURIComponent(newKe)}&tanggal=${tanggal}&penumpang=${jumlahPenumpang}`
    );
  };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-gradient-to-b from-blue-700 to-blue-500 text-white rounded-b-3xl pb-8 shadow-lg">
        ...
        <div className="px-6 pt-2">
          <h1 className="text-2xl font-bold text-center">Pilih Perjalanan Mobil</h1>
          <p className="text-sm text-blue-100 mt-1 text-center">
            Temukan layanan mobil yang sesuai kebutuhanmu
          </p>
        </div>
      </div>
      ...
      <div className="mt-[-20px] px-6">
        <div className="bg-white rounded-xl shadow-lg p-5 space-y-5">
          ...
          <div className="space-y-1">
            ...
            <Link
              href={`/customer/mobil/pilih-lokasi?tipe=dari&dari=${encodeURIComponent(
                lokasiDari
              )}&ke=${encodeURIComponent(lokasiKe)}&tanggal=${tanggal}&penumpang=${jumlahPenumpang}`}
          ...
          <div className="space-y-1">
            ...
            <Link
              href={`/customer/mobil/pilih-penumpang?dari=${encodeURIComponent(
                lokasiDari
              )}&ke=${encodeURIComponent(lokasiKe)}&tanggal=${tanggal}`}
              className="flex items-center gap-2 p-3 border rounded-lg text-sm text-gray-700 hover:bg-gray-100 transition"
            >
              <User2 className="text-blue-600 w-5 h-5" />
              <span>{jumlahPenumpang} Dewasa</span>
            </Link>
          </div>

          {/* Tombol Cari */}
          <button
            onClick={() => router.push("/customer/mobil/hasil")}
