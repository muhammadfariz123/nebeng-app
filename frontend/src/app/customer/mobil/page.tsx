...
export default function MobilSearchPage() {
  ...
  useEffect(() => {
    ...
    if (penumpang) setJumlahPenumpang(parseInt(penumpang));
  }, [searchParams]);

  // ✅ Ambil tanggal tersedia dari backend (hanya untuk mobil)
  useEffect(() => {
  ...
  const handleTukar = () => {
    ...
    setLokasiKe(newKe);
    router.replace(
      `/customer/mobil?dari=${encodeURIComponent(newDari)}&ke=${encodeURIComponent(
        newKe
      )}&tanggal=${tanggal}&penumpang=${jumlahPenumpang}`
    );
  };
  ...
  const handleCari = async () => {
    ...
    try {
      ...
      if (hasil.length === 0) {
        setModalMessage(
          "Maaf, belum ada driver mobil yang menyediakan rute ini. Silakan coba lokasi lain."
        );
        setShowModal(true);
        return;
      }
      router.push(
        `/customer/mobil/hasil?dari=${encodeURIComponent(
          lokasiDari
    ...
    <div className="min-h-screen bg-white relative">
      ...
      <div className="bg-gradient-to-b from-blue-700 to-blue-500 text-white rounded-b-3xl pb-8 shadow-lg">
        ...
        <div className="px-6 pt-2 text-center">
          <h1 className="text-2xl font-bold">Pilih Perjalanan Mobil</h1>
          <p className="text-sm text-blue-100 mt-1">
            Temukan perjalanan mobil yang sesuai dengan kebutuhanmu
          </p>
          <div className="mt-4">
        ...
        <div className="bg-white rounded-xl shadow-lg p-5 space-y-5">
          <div className="bg-blue-50 border border-blue-200 text-blue-800 p-3 rounded-lg flex items-start gap-2 text-sm">
            ...
            <p>
              ...
              <strong>tebengan mobil</strong> yang tersedia.
            </p>
          </div>
          ...
          <div className="space-y-1">
            ...
            <Link
              href={`/customer/mobil/pilih-lokasi?tipe=dari&dari=${encodeURIComponent(
                lokasiDari
              )}&ke=${encodeURIComponent(
                lokasiKe
              )}&tanggal=${tanggal}&penumpang=${jumlahPenumpang}`}
          ...
          <div className="space-y-1">
            ...
            <Link
              href={`/customer/mobil/pilih-lokasi?tipe=ke&dari=${encodeURIComponent(
                lokasiDari
              )}&ke=${encodeURIComponent(
                lokasiKe
              )}&tanggal=${tanggal}&penumpang=${jumlahPenumpang}`}
          ...
          {/* Jumlah Penumpang */}
          <div className="space-y-1">
            <label className="text-xs text-gray-500">Jumlah Penumpang</label>
            <input
              type="number"
              min={1}
              max={6}
