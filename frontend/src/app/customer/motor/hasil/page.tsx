...
export default function HasilPencarianMotorPage() {
  ...
  const [data, setData] = useState<Tebengan[]>([])
  const [filtered, setFiltered] = useState<Tebengan[]>([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [showTrips, setShowTrips] = useState(false)
  const [confirmVisible, setConfirmVisible] = useState(true)

  // 🔹 Ambil data tebengan dari backend
  const fetchData = async () => {
    setLoading(true)
    setError(null)
    try {
      const res = await axios.get<Tebengan[]>('http://localhost:3001/tebengan?type=motor')
      setData(res.data)
      setShowTrips(true)
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="sticky top-0 z-30 bg-white shadow-sm border-b flex items-center px-4 py-3">
        ...
        <h1 className="ml-3 text-base font-semibold text-gray-800">
          Hasil Pencarian Tebengan Motor
        </h1>
      </div>
        ...
        <div className="flex flex-col justify-center items-center flex-1 p-6 text-center">
          <div className="border rounded-xl shadow-md p-6 bg-white w-full max-w-sm">
            ...
            <div className="flex flex-col gap-2 mt-4">
              ...
              <button
                onClick={() => router.push('/customer/motor/pilih-lokasi')}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 bg-white space-y-4">
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((trip) => {
              ...
              const jamTiba = new Date(new Date(trip.waktu).getTime() + 90 * 60000).toLocaleTimeString('id-ID', {
                hour: '2-digit',
                minute: '2-digit',
              })
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-600 mb-2">
                    ...
                    <span>
                      <strong>Jam Keberangkatan:</strong> {jam} — <strong>Perkiraan Tiba:</strong> {jamTiba}
                    </span>
                  </div>
                  ...
                  <div className="flex items-start gap-3 text-sm text-gray-700">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="text-xs text-gray-400">Durasi sekitar 1–2 jam</div>
                      <div>
