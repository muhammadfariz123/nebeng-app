...
export default function PilihLokasiBarangPage() {
  ...
  const handleSelect = (lokasi: string) => {
    const newQuery =
      tipe === 'dari'
        ? `?dari=${encodeURIComponent(lokasi)}&ke=${encodeURIComponent(ke)}&tanggal=${tanggal}&barang=${encodeURIComponent(barang)}`
        : `?dari=${encodeURIComponent(dari)}&ke=${encodeURIComponent(lokasi)}&tanggal=${tanggal}&barang=${encodeURIComponent(barang)}`

    router.push(`/customer/barang${newQuery}`)
  }
      ...
      <div className="px-4 pt-6 flex items-center gap-3">
        ...
        <div className="flex-1 bg-gray-100 rounded-full px-4 py-2 flex items-center gap-2">
          ...
          <input
            ...
            // Optional: tambahkan pencarian jika dibutuhkan
          />
        </div>
      </div>
